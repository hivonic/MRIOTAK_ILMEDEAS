...
if __name__ == "__main__":
    ...
    chosen_model.save('brain_mri_mobilenetv2_seimbang.h5')
    print("Model saved as 'brain_mri_mobilenetv2_seimbang.h5'")
    ...
    with open('roc_curve_train_data_mobilenetv2_seimbang.pkl', 'wb') as f:
    ...
    print("Training ROC curve data saved as 'roc_curve_train_data_mobilenetv2_seimbang.pkl'")
    
    # Confusion Matrix untuk Training
    cm_train = confusion_matrix(y_true_train, y_pred_train)
    np.save('confusion_matrix_train_mobilenetv2_seimbang.npy', cm_train)
    print("Training confusion matrix saved as 'confusion_matrix_train_mobilenetv2_seimbang.npy'")
    ...
    with open('roc_curve_test_data_mobilenetv2_seimbang.pkl', 'wb') as f:
        pickle.dump({'fpr': fpr_test, 'tpr': tpr_test, 'thresholds': thresholds_test, 'auc': roc_auc_test}, f)
    print(f"Test ROC AUC: {roc_auc_test:.4f}")
    print("Test ROC curve data saved as 'roc_curve_test_data_mobilenetv2_seimbang.pkl'")
    
    # Confusion Matrix untuk Test
    cm_test = confusion_matrix(y_true_test, y_pred_test)
    np.save('confusion_matrix_test_mobilenetv2_seimbang.npy', cm_test)
    print("Test confusion matrix saved as 'confusion_matrix_test_mobilenetv2_seimbang.npy'")
    ...
    plt.savefig('roc_curve_training_mobilenetv2_seimbang.png', dpi=300, bbox_inches='tight')
    plt.close()
    print("Training ROC curve saved as 'roc_curve_training_mobilenetv2_seimbang.png'")
    ...
    plt.legend(loc='lower right')
    plt.grid(True, alpha=0.3)
    plt.savefig('roc_curve_test_mobilenetv2_seimbang.png', dpi=300, bbox_inches='tight')
    plt.close()
    print("Test ROC curve saved as 'roc_curve_test_mobilenetv2_seimbang.png'")
    ...
    plt.savefig('confusion_matrix_training_mobilenetv2_seimbang.png', dpi=300, bbox_inches='tight')
    plt.close()
    print("Training confusion matrix saved as 'confusion_matrix_training_mobilenetv2_seimbang.png'")
    ...
    plt.ylabel('True Label')
    plt.xlabel('Predicted Label')  
    plt.savefig('confusion_matrix_test_mobilenetv2_seimbang.png', dpi=300, bbox_inches='tight')
    plt.close()
    print("Test confusion matrix saved as 'confusion_matrix_test_mobilenetv2_seimbang.png'")
    ...
    roc_train_df.to_csv('roc_curve_train_data_mobilenetv2_seimbang.csv', index=False)
    
    # ROC Training - NPY
    np.save('roc_curve_train_fpr_mobilenetv2_seimbang.npy', fpr_train)
    np.save('roc_curve_train_tpr_mobilenetv2_seimbang.npy', tpr_train)
    np.save('roc_curve_train_thresholds_mobilenetv2_seimbang.npy', thresholds_train)
    
    print(f"Training ROC AUC: {roc_auc_train:.4f}")
    print("Training ROC curve saved in multiple formats:")
    print("  - roc_curve_train_data_mobilenetv2_seimbang.pkl")
    print("  - roc_curve_train_data_mobilenetv2_seimbang.csv")
    print("  - roc_curve_train_fpr_mobilenetv2_seimbang.npy, roc_curve_train_tpr_mobilenetv2_seimbang.npy, roc_curve_train_thresholds_mobilenetv2_seimbang.npy")
    print("  - roc_curve_training_mobilenetv2_seimbang.png")

    # === SAVE CONFUSION MATRIX TRAINING IN MULTIPLE FORMATS ===
    # CM Training - PKL
    with open('confusion_matrix_train_data_mobilenetv2_seimbang.pkl', 'wb') as f:
        pickle.dump(cm_train, f)
    
    # CM Training - CSV
    cm_train_df = pd.DataFrame(cm_train, index=['Healthy', 'Tumor'], columns=['Healthy', 'Tumor'])
    cm_train_df.to_csv('confusion_matrix_train_data_mobilenetv2_seimbang.csv')
    
    print("Training confusion matrix saved in multiple formats:")
    print("  - confusion_matrix_train_mobilenetv2_seimbang.npy")
    print("  - confusion_matrix_train_data_mobilenetv2_seimbang.pkl")
    print("  - confusion_matrix_train_data_mobilenetv2_seimbang.csv")
    print("  - confusion_matrix_training_mobilenetv2_seimbang.png")
    ...
    roc_test_df.to_csv('roc_curve_test_data_mobilenetv2_seimbang.csv', index=False)
    
    # ROC Test - NPY
    np.save('roc_curve_test_fpr_mobilenetv2_seimbang.npy', fpr_test)
    np.save('roc_curve_test_tpr_mobilenetv2_seimbang.npy', tpr_test)
    np.save('roc_curve_test_thresholds_mobilenetv2_seimbang.npy', thresholds_test)
    
    print(f"Test ROC AUC: {roc_auc_test:.4f}")
    print("Test ROC curve saved in multiple formats:")
    print("  - roc_curve_test_data_mobilenetv2_seimbang.pkl")
    print("  - roc_curve_test_data_mobilenetv2_seimbang.csv")
    print("  - roc_curve_test_fpr_mobilenetv2_seimbang.npy, roc_curve_test_tpr_mobilenetv2_seimbang.npy, roc_curve_test_thresholds_mobilenetv2_seimbang.npy")
    print("  - roc_curve_test_mobilenetv2_seimbang.png")

    # === SAVE CONFUSION MATRIX TEST IN MULTIPLE FORMATS ===
    # CM Test - PKL
    with open('confusion_matrix_test_data_mobilenetv2_seimbang.pkl', 'wb') as f:
        pickle.dump(cm_test, f)
    
    # CM Test - CSV
    cm_test_df = pd.DataFrame(cm_test, index=['Healthy', 'Tumor'], columns=['Healthy', 'Tumor'])
    cm_test_df.to_csv('confusion_matrix_test_data_mobilenetv2_seimbang.csv')
    
    print("Test confusion matrix saved in multiple formats:")
    print("  - confusion_matrix_test_mobilenetv2_seimbang.npy")
    print("  - confusion_matrix_test_data_mobilenetv2_seimbang.pkl")
    print("  - confusion_matrix_test_data_mobilenetv2_seimbang.csv")
    print("  - confusion_matrix_test_mobilenetv2_seimbang.png")

    # === SAVE TRAINING HISTORY IN MULTIPLE FORMATS ===
    if history:
        # Training History - PKL
        with open('training_history_data_mobilenetv2_seimbang.pkl', 'wb') as f:
            pickle.dump(history.history, f)
        
        # Training History - CSV
        history_df = pd.DataFrame(history.history)
        history_df.to_csv('training_history_data_mobilenetv2_seimbang.csv', index=False)
        
        # Training History - NPY
        np.save('training_history_data_mobilenetv2_seimbang.npy', history.history)
        ...
        plt.tight_layout()
        plt.savefig('training_history_mobilenetv2_seimbang.png', dpi=300, bbox_inches='tight')
        plt.close()
        
        print("Training history saved in multiple formats:")
        print("  - training_history_data_mobilenetv2_seimbang.pkl")
        print("  - training_history_data_mobilenetv2_seimbang.csv")
        print("  - training_history_data_mobilenetv2_seimbang.npy")
        print("  - training_history_mobilenetv2_seimbang.png")

    # === SAVE AUC DATA IN MULTIPLE FORMATS ===
    # AUC Training
    auc_train_data = {'auc': roc_auc_train, 'fpr': fpr_train, 'tpr': tpr_train}
    with open('auc_train_data_mobilenetv2_seimbang.pkl', 'wb') as f:
    ...
    auc_train_df.to_csv('auc_train_data_mobilenetv2_seimbang.csv', index=False)
    np.save('auc_train_value_mobilenetv2_seimbang.npy', roc_auc_train)
    
    plt.figure(figsize=(6, 4))
    plt.bar(['Training AUC'], [roc_auc_train], color='blue', alpha=0.7)
    plt.ylim(0, 1)
    plt.ylabel('AUC Score')
    plt.title('Training AUC Score - MobileNetV2 (Balanced Dataset)')
    plt.text(0, roc_auc_train + 0.02, f'{roc_auc_train:.4f}', ha='center', fontweight='bold')
    plt.grid(True, alpha=0.3)
    plt.savefig('auc_curve_training_mobilenetv2_seimbang.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    print("Training AUC saved in multiple formats:")
    print("  - auc_train_data_mobilenetv2_seimbang.pkl")
    print("  - auc_train_data_mobilenetv2_seimbang.csv")
    print("  - auc_train_value_mobilenetv2_seimbang.npy")
    print("  - auc_curve_training_mobilenetv2_seimbang.png")

    # AUC Test
    auc_test_data = {'auc': roc_auc_test, 'fpr': fpr_test, 'tpr': tpr_test}
    with open('auc_test_data_mobilenetv2_seimbang.pkl', 'wb') as f:
    ...
    auc_test_df.to_csv('auc_test_data_mobilenetv2_seimbang.csv', index=False)
    np.save('auc_test_value_mobilenetv2_seimbang.npy', roc_auc_test)
    
    plt.figure(figsize=(6, 4))
    plt.bar(['Test AUC'], [roc_auc_test], color='red', alpha=0.7)
    plt.ylim(0, 1)
    plt.ylabel('AUC Score')
    plt.title('Test AUC Score - MobileNetV2 (Balanced Dataset)')
    plt.text(0, roc_auc_test + 0.02, f'{roc_auc_test:.4f}', ha='center', fontweight='bold')
    plt.grid(True, alpha=0.3)
    plt.savefig('auc_curve_test_mobilenetv2_seimbang.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    print("Test AUC saved in multiple formats:")
    print("  - auc_test_data_mobilenetv2_seimbang.pkl")
    print("  - auc_test_data_mobilenetv2_seimbang.csv")
    print("  - auc_test_value_mobilenetv2_seimbang.npy")
    print("  - auc_curve_test_mobilenetv2_seimbang.png")
    ...
    auc_combined_df.to_csv('auc_combined_data_mobilenetv2_seimbang.csv', index=False)
    print("Combined AUC data saved as 'auc_combined_data_mobilenetv2_seimbang.csv'")
