        ...
        for filename in os.listdir(folder_path):
            if filename.lower().endswith(supported_formats):
                ...
                try:
                    ...
                    # Normalization is now handled by preprocess_input in the model (This comment was already there)
...
if __name__ == "__main__":
    ...
    base_output_dir = "Output Inception Seimbang"
    output_dirs = {
    ...
    chosen_model.save(os.path.join(output_dirs['models'], 'brain_mri_inceptionv3_seimbang.h5'))
    print(f"Model saved as '{os.path.join(output_dirs['models'], 'brain_mri_inceptionv3_seimbang.h5')}'")
    ...
    plt.title('ROC Curve - Training Set - InceptionV3 (Balanced)')
    plt.legend(loc='lower right')
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(output_dirs['roc'], 'roc_curve_training_inceptionv3_seimbang.png'), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
    plt.title('ROC Curve - Test Set - InceptionV3 (Balanced)')
    plt.legend(loc='lower right')
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(output_dirs['roc'], 'roc_curve_test_inceptionv3_seimbang.png'), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.title('Precision-Recall Curve - Training Set - InceptionV3 (Balanced)')
    plt.legend(loc='lower left')
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(output_dirs['pr'], 'pr_curve_training_inceptionv3_seimbang.png'), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.xlabel('Recall')
    plt.ylabel('Precision')
    plt.title('Precision-Recall Curve - Test Set - InceptionV3 (Balanced)')
    plt.legend(loc='lower left')
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(output_dirs['pr'], 'pr_curve_test_inceptionv3_seimbang.png'), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.title('Confusion Matrix - Training Set - InceptionV3 (Balanced)')
    plt.ylabel('True Label')
    plt.xlabel('Predicted Label')
    plt.savefig(os.path.join(output_dirs['confusion'], 'confusion_matrix_training_inceptionv3_seimbang.png'), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    sns.heatmap(cm_test, annot=True, fmt="d", cmap='Blues',
                xticklabels=class_names, yticklabels=class_names)
    plt.title('Confusion Matrix - Test Set - InceptionV3 (Balanced)')
    plt.ylabel('True Label')
    plt.xlabel('Predicted Label')
    plt.savefig(os.path.join(output_dirs['confusion'], 'confusion_matrix_test_inceptionv3_seimbang.png'), dpi=300, bbox_inches='tight')
    plt.close()

    # === SAVE ROC CURVE DATA ===
    # ROC Training - Multiple formats
    with open(os.path.join(output_dirs['roc'], 'roc_curve_train_data_inceptionv3_seimbang.pkl'), 'wb') as f:
    ...
    roc_train_df.to_csv(os.path.join(output_dirs['roc'], 'roc_curve_train_data_inceptionv3_seimbang.csv'), index=False)
    
    np.save(os.path.join(output_dirs['roc'], 'roc_curve_train_fpr_inceptionv3_seimbang.npy'), fpr_train)
    np.save(os.path.join(output_dirs['roc'], 'roc_curve_train_tpr_inceptionv3_seimbang.npy'), tpr_train)
    np.save(os.path.join(output_dirs['roc'], 'roc_curve_train_thresholds_inceptionv3_seimbang.npy'), thresholds_train)

    # ROC Test - Multiple formats
    with open(os.path.join(output_dirs['roc'], 'roc_curve_test_data_inceptionv3_seimbang.pkl'), 'wb') as f:
    ...
    roc_test_df.to_csv(os.path.join(output_dirs['roc'], 'roc_curve_test_data_inceptionv3_seimbang.csv'), index=False)
    
    np.save(os.path.join(output_dirs['roc'], 'roc_curve_test_fpr_inceptionv3_seimbang.npy'), fpr_test)
    np.save(os.path.join(output_dirs['roc'], 'roc_curve_test_tpr_inceptionv3_seimbang.npy'), tpr_test)
    np.save(os.path.join(output_dirs['roc'], 'roc_curve_test_thresholds_inceptionv3_seimbang.npy'), thresholds_test)

    # === SAVE PR CURVE DATA ===
    # PR Training - Multiple formats
    with open(os.path.join(output_dirs['pr'], 'pr_curve_train_data_inceptionv3_seimbang.pkl'), 'wb') as f:
    ...
    pr_train_df.to_csv(os.path.join(output_dirs['pr'], 'pr_curve_train_data_inceptionv3_seimbang.csv'), index=False)
    
    np.save(os.path.join(output_dirs['pr'], 'pr_curve_train_precision_inceptionv3_seimbang.npy'), precision_train)
    np.save(os.path.join(output_dirs['pr'], 'pr_curve_train_recall_inceptionv3_seimbang.npy'), recall_train)
    np.save(os.path.join(output_dirs['pr'], 'pr_curve_train_thresholds_inceptionv3_seimbang.npy'), pr_thresholds_train)

    # PR Test - Multiple formats
    with open(os.path.join(output_dirs['pr'], 'pr_curve_test_data_inceptionv3_seimbang.pkl'), 'wb') as f:
    ...
    pr_test_df.to_csv(os.path.join(output_dirs['pr'], 'pr_curve_test_data_inceptionv3_seimbang.csv'), index=False)
    
    np.save(os.path.join(output_dirs['pr'], 'pr_curve_test_precision_inceptionv3_seimbang.npy'), precision_test)
    np.save(os.path.join(output_dirs['pr'], 'pr_curve_test_recall_inceptionv3_seimbang.npy'), recall_test)
    np.save(os.path.join(output_dirs['pr'], 'pr_curve_test_thresholds_inceptionv3_seimbang.npy'), pr_thresholds_test)

    # === SAVE CONFUSION MATRIX DATA ===
    # CM Training
    np.save(os.path.join(output_dirs['confusion'], 'confusion_matrix_train_inceptionv3_seimbang.npy'), cm_train)
    with open(os.path.join(output_dirs['confusion'], 'confusion_matrix_train_data_inceptionv3_seimbang.pkl'), 'wb') as f:
        pickle.dump(cm_train, f)
    cm_train_df = pd.DataFrame(cm_train, index=['Healthy', 'Tumor'], columns=['Healthy', 'Tumor'])
    cm_train_df.to_csv(os.path.join(output_dirs['confusion'], 'confusion_matrix_train_data_inceptionv3_seimbang.csv'))

    # CM Test
    np.save(os.path.join(output_dirs['confusion'], 'confusion_matrix_test_inceptionv3_seimbang.npy'), cm_test)
    with open(os.path.join(output_dirs['confusion'], 'confusion_matrix_test_data_inceptionv3_seimbang.pkl'), 'wb') as f:
        pickle.dump(cm_test, f)
    cm_test_df = pd.DataFrame(cm_test, index=['Healthy', 'Tumor'], columns=['Healthy', 'Tumor'])
    cm_test_df.to_csv(os.path.join(output_dirs['confusion'], 'confusion_matrix_test_data_inceptionv3_seimbang.csv'))

    # === SAVE TRAINING HISTORY ===
    if history:
        with open(os.path.join(output_dirs['history'], 'training_history_data_inceptionv3_seimbang.pkl'), 'wb') as f:
            pickle.dump(history.history, f)
        
        history_df = pd.DataFrame(history.history)
        history_df.to_csv(os.path.join(output_dirs['history'], 'training_history_data_inceptionv3_seimbang.csv'), index=False)
        
        np.save(os.path.join(output_dirs['history'], 'training_history_data_inceptionv3_seimbang.npy'), history.history)
        ...
        plt.title('Model Accuracy - InceptionV3 (Balanced Dataset)')
        plt.ylabel('Accuracy')
        plt.xlabel('Epoch')
        plt.legend()
        plt.grid(True, alpha=0.3)
        ...
        plt.title('Model Loss - InceptionV3 (Balanced Dataset)')
        plt.ylabel('Loss')
        plt.xlabel('Epoch')
        plt.legend()
        plt.grid(True, alpha=0.3)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dirs['history'], 'training_history_inceptionv3_seimbang.png'), dpi=300, bbox_inches='tight')
        plt.close()

    # === SAVE ROC AUC DATA ===
    # ROC AUC Training
    auc_train_data = {'auc': roc_auc_train, 'fpr': fpr_train, 'tpr': tpr_train}
    with open(os.path.join(output_dirs['auc'], 'auc_train_data_inceptionv3_seimbang.pkl'), 'wb') as f:
    ...
    auc_train_df.to_csv(os.path.join(output_dirs['auc'], 'auc_train_data_inceptionv3_seimbang.csv'), index=False)
    np.save(os.path.join(output_dirs['auc'], 'auc_train_value_inceptionv3_seimbang.npy'), roc_auc_train)
    ...
    plt.title('Training ROC AUC Score - InceptionV3 (Balanced)')
    plt.text(0, roc_auc_train + 0.02, f'{roc_auc_train:.4f}', ha='center', fontweight='bold')
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(output_dirs['auc'], 'auc_curve_training_inceptionv3_seimbang.png'), dpi=300, bbox_inches='tight')
    plt.close()

    # ROC AUC Test
    auc_test_data = {'auc': roc_auc_test, 'fpr': fpr_test, 'tpr': tpr_test}
    with open(os.path.join(output_dirs['auc'], 'auc_test_data_inceptionv3_seimbang.pkl'), 'wb') as f:
    ...
    auc_test_df.to_csv(os.path.join(output_dirs['auc'], 'auc_test_data_inceptionv3_seimbang.csv'), index=False)
    np.save(os.path.join(output_dirs['auc'], 'auc_test_value_inceptionv3_seimbang.npy'), roc_auc_test)
    ...
    plt.ylim(0, 1)
    plt.ylabel('ROC AUC Score')
    plt.title('Test ROC AUC Score - InceptionV3 (Balanced)')
    plt.text(0, roc_auc_test + 0.02, f'{roc_auc_test:.4f}', ha='center', fontweight='bold')
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(output_dirs['auc'], 'auc_curve_test_inceptionv3_seimbang.png'), dpi=300, bbox_inches='tight')
    plt.close()

    # === SAVE PR AUC DATA ===
    # PR AUC Training
    pr_auc_train_data = {'pr_auc': pr_auc_train, 'precision': precision_train, 'recall': recall_train}
    with open(os.path.join(output_dirs['pr_auc'], 'pr_auc_train_data_inceptionv3_seimbang.pkl'), 'wb') as f:
    ...
    pr_auc_train_df.to_csv(os.path.join(output_dirs['pr_auc'], 'pr_auc_train_data_inceptionv3_seimbang.csv'), index=False)
    np.save(os.path.join(output_dirs['pr_auc'], 'pr_auc_train_value_inceptionv3_seimbang.npy'), pr_auc_train)
    ...
    plt.title('Training PR AUC Score - InceptionV3 (Balanced)')
    plt.text(0, pr_auc_train + 0.02, f'{pr_auc_train:.4f}', ha='center', fontweight='bold')
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(output_dirs['pr_auc'], 'pr_auc_curve_training_inceptionv3_seimbang.png'), dpi=300, bbox_inches='tight')
    plt.close()

    # PR AUC Test
    pr_auc_test_data = {'pr_auc': pr_auc_test, 'precision': precision_test, 'recall': recall_test}
    with open(os.path.join(output_dirs['pr_auc'], 'pr_auc_test_data_inceptionv3_seimbang.pkl'), 'wb') as f:
    ...
    pr_auc_test_df.to_csv(os.path.join(output_dirs['pr_auc'], 'pr_auc_test_data_inceptionv3_seimbang.csv'), index=False)
    np.save(os.path.join(output_dirs['pr_auc'], 'pr_auc_test_value_inceptionv3_seimbang.npy'), pr_auc_test)
    ...
    plt.ylim(0, 1)
    plt.ylabel('PR AUC Score')
    plt.title('Test PR AUC Score - InceptionV3 (Balanced)')
    plt.text(0, pr_auc_test + 0.02, f'{pr_auc_test:.4f}', ha='center', fontweight='bold')
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(output_dirs['pr_auc'], 'pr_auc_curve_test_inceptionv3_seimbang.png'), dpi=300, bbox_inches='tight')
    plt.close()

    # === SUMMARY METRICS ===
    print("\n=== PERFORMANCE SUMMARY - INCEPTIONV3 (BALANCED DATASET) ===")
    print("TRAINING SET:")
