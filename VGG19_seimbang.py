...
if __name__ == "__main__":
    ...
    chosen_model.save('brain_mri_vgg19_seimbang.h5')
    print("Model saved as 'brain_mri_vgg19_seimbang.h5'")
    ...
    with open('roc_curve_train_data_seimbang.pkl', 'wb') as f:
    ...
    print("Training ROC curve data saved as 'roc_curve_train_data_seimbang.pkl'")
    
    # Confusion Matrix untuk Training
    cm_train = confusion_matrix(y_true_train, y_pred_train)
    np.save('confusion_matrix_train_seimbang.npy', cm_train)
    print("Training confusion matrix saved as 'confusion_matrix_train_seimbang.npy'")
    ...
    with open('roc_curve_test_data_seimbang.pkl', 'wb') as f:
        pickle.dump({'fpr': fpr_test, 'tpr': tpr_test, 'thresholds': thresholds_test, 'auc': roc_auc_test}, f)
    print(f"Test ROC AUC: {roc_auc_test:.4f}")
    print("Test ROC curve data saved as 'roc_curve_test_data_seimbang.pkl'")
    
    # Confusion Matrix untuk Test
    cm_test = confusion_matrix(y_true_test, y_pred_test)
    np.save('confusion_matrix_test_seimbang.npy', cm_test)
    print("Test confusion matrix saved as 'confusion_matrix_test_seimbang.npy'")
    ...
    plt.title('ROC Curve - Training Set (Balanced)')
    plt.legend(loc='lower right')
    plt.grid(True, alpha=0.3)
    plt.savefig('roc_curve_training_seimbang.png', dpi=300, bbox_inches='tight')
    plt.close()
    print("Training ROC curve saved as 'roc_curve_training_seimbang.png'")
    ...
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
    plt.title('ROC Curve - Test Set (Balanced)')
    plt.legend(loc='lower right')
    plt.grid(True, alpha=0.3)
    plt.savefig('roc_curve_test_seimbang.png', dpi=300, bbox_inches='tight')
    plt.close()
    print("Test ROC curve saved as 'roc_curve_test_seimbang.png'")
    ...
    plt.title('Confusion Matrix - Training Set (Balanced)')
    plt.ylabel('True Label')
    plt.xlabel('Predicted Label')
    plt.savefig('confusion_matrix_training_seimbang.png', dpi=300, bbox_inches='tight')
    plt.close()
    print("Training confusion matrix saved as 'confusion_matrix_training_seimbang.png'")
    ...
    plt.ylabel('True Label')
    plt.xlabel('Predicted Label')
    plt.savefig('confusion_matrix_test_seimbang.png', dpi=300, bbox_inches='tight')
    plt.close()
    print("Test confusion matrix saved as 'confusion_matrix_test_seimbang.png'")
    ...
    roc_train_df.to_csv('roc_curve_train_data_seimbang.csv', index=False)
    
    # ROC Training - NPY
    np.save('roc_curve_train_fpr_seimbang.npy', fpr_train)
    np.save('roc_curve_train_tpr_seimbang.npy', tpr_train)
    np.save('roc_curve_train_thresholds_seimbang.npy', thresholds_train)
    
    print(f"Training ROC AUC: {roc_auc_train:.4f}")
    print("Training ROC curve saved in multiple formats:")
    print("  - roc_curve_train_data_seimbang.pkl")
    print("  - roc_curve_train_data_seimbang.csv")
    print("  - roc_curve_train_fpr_seimbang.npy, roc_curve_train_tpr_seimbang.npy, roc_curve_train_thresholds_seimbang.npy")
    print("  - roc_curve_training_seimbang.png")
    ...
    # CM Training - PKL
    with open('confusion_matrix_train_data_seimbang.pkl', 'wb') as f:
        pickle.dump(cm_train, f)
    
    # CM Training - CSV
    cm_train_df = pd.DataFrame(cm_train, index=['Healthy', 'Tumor'], columns=['Healthy', 'Tumor'])
    cm_train_df.to_csv('confusion_matrix_train_data_seimbang.csv')
    
    print("Training confusion matrix saved in multiple formats:")
    print("  - confusion_matrix_train_seimbang.npy")
    print("  - confusion_matrix_train_data_seimbang.pkl")
    print("  - confusion_matrix_train_data_seimbang.csv")
    print("  - confusion_matrix_training_seimbang.png")
    ...
    roc_test_df.to_csv('roc_curve_test_data_seimbang.csv', index=False)
    
    # ROC Test - NPY
    np.save('roc_curve_test_fpr_seimbang.npy', fpr_test)
    np.save('roc_curve_test_tpr_seimbang.npy', tpr_test)
    np.save('roc_curve_test_thresholds_seimbang.npy', thresholds_test)
    
    print(f"Test ROC AUC: {roc_auc_test:.4f}")
    print("Test ROC curve saved in multiple formats:")
    print("  - roc_curve_test_data_seimbang.pkl")
    print("  - roc_curve_test_data_seimbang.csv")
    print("  - roc_curve_test_fpr_seimbang.npy, roc_curve_test_tpr_seimbang.npy, roc_curve_test_thresholds_seimbang.npy")
    print("  - roc_curve_test_seimbang.png")
    ...
    # CM Test - PKL
    with open('confusion_matrix_test_data_seimbang.pkl', 'wb') as f:
        pickle.dump(cm_test, f)
    
    # CM Test - CSV
    cm_test_df = pd.DataFrame(cm_test, index=['Healthy', 'Tumor'], columns=['Healthy', 'Tumor'])
    cm_test_df.to_csv('confusion_matrix_test_data_seimbang.csv')
    
    print("Test confusion matrix saved in multiple formats:")
    print("  - confusion_matrix_test_seimbang.npy")
    print("  - confusion_matrix_test_data_seimbang.pkl")
    print("  - confusion_matrix_test_data_seimbang.csv")
    print("  - confusion_matrix_test_seimbang.png")

    # === SAVE TRAINING HISTORY IN MULTIPLE FORMATS ===
    if history:
        # Training History - PKL
        with open('training_history_data_seimbang.pkl', 'wb') as f:
            pickle.dump(history.history, f)
        
        # Training History - CSV
        history_df = pd.DataFrame(history.history)
        history_df.to_csv('training_history_data_seimbang.csv', index=False)
        
        # Training History - NPY
        np.save('training_history_data_seimbang.npy', history.history)
        ...
        plt.tight_layout()
        plt.savefig('training_history_seimbang.png', dpi=300, bbox_inches='tight')
        plt.close()
        
        print("Training history saved in multiple formats:")
        print("  - training_history_data_seimbang.pkl")
        print("  - training_history_data_seimbang.csv")
        print("  - training_history_data_seimbang.npy")
        print("  - training_history_seimbang.png")

    # === SAVE AUC DATA IN MULTIPLE FORMATS ===
    # AUC Training - PKL
    auc_train_data = {'auc': roc_auc_train, 'fpr': fpr_train, 'tpr': tpr_train}
    with open('auc_train_data_seimbang.pkl', 'wb') as f:
    ...
    auc_train_df.to_csv('auc_train_data_seimbang.csv', index=False)
    
    # AUC Training - NPY
    np.save('auc_train_value_seimbang.npy', roc_auc_train)
    ...
    plt.title('Training AUC Score (Balanced Dataset)')
    plt.text(0, roc_auc_train + 0.02, f'{roc_auc_train:.4f}', ha='center', fontweight='bold')
    plt.grid(True, alpha=0.3)
    plt.savefig('auc_curve_training_seimbang.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    print("Training AUC saved in multiple formats:")
    print("  - auc_train_data_seimbang.pkl")
    print("  - auc_train_data_seimbang.csv")
    print("  - auc_train_value_seimbang.npy")
    print("  - auc_curve_training_seimbang.png")

    # AUC Test - PKL
    auc_test_data = {'auc': roc_auc_test, 'fpr': fpr_test, 'tpr': tpr_test}
    with open('auc_test_data_seimbang.pkl', 'wb') as f:
    ...
    auc_test_df.to_csv('auc_test_data_seimbang.csv', index=False)
    
    # AUC Test - NPY
    np.save('auc_test_value_seimbang.npy', roc_auc_test)
    ...
    plt.ylim(0, 1)
    plt.ylabel('AUC Score')
    plt.title('Test AUC Score (Balanced Dataset)')
    plt.text(0, roc_auc_test + 0.02, f'{roc_auc_test:.4f}', ha='center', fontweight='bold')
    plt.grid(True, alpha=0.3)
    plt.savefig('auc_curve_test_seimbang.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    print("Test AUC saved in multiple formats:")
    print("  - auc_test_data_seimbang.pkl")
    print("  - auc_test_data_seimbang.csv")
    print("  - auc_test_value_seimbang.npy")
    print("  - auc_curve_test_seimbang.png")
    ...
    auc_combined_df.to_csv('auc_combined_data_seimbang.csv', index=False)
    print("Combined AUC data saved as 'auc_combined_data_seimbang.csv'")

    # === SUMMARY METRICS ===
    print("\n=== PERFORMANCE SUMMARY (BALANCED DATASET) ===")
    print("TRAINING SET:")
